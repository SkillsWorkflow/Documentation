...
2. **[Rest](../actions.md#rest)** (`GetData`) fetches the rows to export — here, from a named analytics query.
3. **[Csv](../actions.md#csv)** (`CreateCsv`) turns those rows into a CSV, choosing which columns to include with `dataColumns`.
4. **[Rest](../actions.md#rest)** (`DeliverFile`) sends the file on, attaching it as Base64 with the `ToBase64` pipe: `{{['CreateCsv']$ | ToBase64}}`.
5. **[Result](../actions.md#result)** returns the outcome.
...
:::tip Adapt it
Change the query URL and `dataColumns` to your data, and point `DeliverFile` at wherever the file should go. To email the file instead, swap `DeliverFile` for an [E-mail](../actions.md#e-mail) action — see [Send an email notification](./email-notification.md).
:::
