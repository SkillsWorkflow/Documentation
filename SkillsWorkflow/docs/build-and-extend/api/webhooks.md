...
Webhooks can also act as entry points for [Automation Workflows](../automations/). Instead of sending the event to an external URL, enable **Automation** and select an **Automation Workflow** as the destination.
...
For the automation model, expression syntax and available actions, see [Automations](../automations/) and the [Actions reference](../automations/actions).
