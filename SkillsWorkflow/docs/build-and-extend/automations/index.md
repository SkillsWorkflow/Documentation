...
A trigger can come from a Skills Workflow event or from a [webhook](../api/webhooks.md) configured to run an Automation Workflow. This is useful when an event should start a workflow that calls external systems, transforms data, sends notifications, or returns a structured response.
